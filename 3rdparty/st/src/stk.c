...
__thread _st_clist_t _st_free_stacks; // = ST_INIT_STATIC_CLIST(&_st_free_stacks);
...
void _st_stack_free(_st_stack_t *ts)
{
  ...
  // warning never free the stack
#if 0
  if(_st_num_free_stacks > MAX_ST_FREE_STACK)
  {
...
void _st_init_stack()
{
    _st_free_stacks.prev = &_st_free_stacks;
    _st_free_stacks.next = &_st_free_stacks;
}
