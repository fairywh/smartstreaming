 ...
 * log 打印
 */
typedef int (*_st_log_print)(const char*, ...);
...
struct _st_thread {
  ...
  st_utime_t add_queue_time;     /* Time add to runnable */

  st_utime_t run_time;           /* actually begin run */
};
...
typedef struct _st_vp {
  ...
  st_utime_t stat_schedure_time;  /* 统计开始时间 */
  struct _st_vp_stat_t stats;
#endif
} _st_vp_t;
...
#define _ST_ADD_RUNQ(_thr) \
    ...
    _thr->add_queue_time = _ST_LAST_CLOCK
