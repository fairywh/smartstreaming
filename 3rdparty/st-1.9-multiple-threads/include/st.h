...
typedef struct _st_vp_stat_t {
    unsigned int run_queue_10us;   /* 10us */
    unsigned int run_queue_100us;  /* 100us */
    unsigned int run_queue_1ms;
    unsigned int run_queue_10ms;
    unsigned int run_queue_100ms;
    unsigned int run_queue_1s;
    unsigned int run_queue_large;

    unsigned int vp_cycle_num;
    unsigned int vp_cycle_elapsed;
} _st_vp_stat_t;
...
#define ST_TRACER_NUM 100
typedef struct _st_tracer {
    st_thread_t thrds[ST_TRACER_NUM];
    int index;
} st_tracer_t;

typedef struct _st_thread_info_t {
    st_thread_t trd;
    char *stk_bottom;
    char *stk_top;
    char *sp;
...
typedef struct _st_vp_stat_t* st_vp_stat_t;
typedef struct _st_thread_info_t st_thread_info_t;
#endif
...
extern int st_thread_self_id();
extern int st_thread_id(st_thread_t t);
extern void st_thread_self_setid(int id);
extern void st_thread_self_info(st_thread_info_t* info);
extern void st_thread_tracer(st_tracer_t** tracer);
...
extern st_netfd_t st_netfd_open_socket_only(int osfd);
 ...
 * log 打印
 */
typedef int (*st_log_print)(const char*, ...);

extern int set_st_log_handler(st_log_print handler);
...
#ifdef ST_SCHEDURE_STAT
extern st_vp_stat_t st_get_vp_stat();
#endif
