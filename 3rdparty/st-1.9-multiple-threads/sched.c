...
int default_log_print(const char* format, ...)
{
    return 0;
}

_st_log_print _log_print = default_log_print;

int set_st_log_handler(_st_log_print handler)
{
    if(handler)
    {
        _log_print = handler;
...
void _st_vp_schedule(void)
{
  ...
  if (_ST_RUNQ.next != &_ST_RUNQ) {
    ...
    _ST_DEL_RUNQ(thread);
#ifdef ST_SCHEDURE_STAT
    thread->run_time = _ST_LAST_CLOCK;
    cost_time = (int) (thread->run_time - thread->add_queue_time);
    if(cost_time < 10)
    {
        ++_st_this_vp.stats.run_queue_10us;
    }
    else if(cost_time < 100)
    {
        ++_st_this_vp.stats.run_queue_100us;
    }
    else if(cost_time < 1000)
    {
        ++_st_this_vp.stats.run_queue_1ms;
    }
    else if(cost_time < 10000)
    {
        ++_st_this_vp.stats.run_queue_10ms;
    }
    else if(cost_time < 100000)
    {
        ++_st_this_vp.stats.run_queue_100ms;
    }
    else if(cost_time < 1000000)
    {
        ++_st_this_vp.stats.run_queue_1s;
    }
    else
    {
        ++_st_this_vp.stats.run_queue_large;
    }
#if 0
    if(_ST_LAST_CLOCK - _st_this_vp.stat_schedure_time >  60*1000000)
    {
        sprintf(buf, "st stat schedure: 10us=%u, 100us=%u, 1ms=%u, 10ms=%u, 100ms=%u, 1s=%u, large=%u.",
            _st_this_vp.stats.run_queue_10us,
            _st_this_vp.stats.run_queue_100us,
            _st_this_vp.stats.run_queue_1ms,
            _st_this_vp.stats.run_queue_10ms,
            ...
            _st_this_vp.stats.run_queue_large);
        _log_print(buf);
        _st_this_vp.stat_schedure_time = _ST_LAST_CLOCK;
        _st_this_vp.stats.run_queue_10us = 0;
        _st_this_vp.stats.run_queue_100us = 0;
        _st_this_vp.stats.run_queue_1ms = 0;
        _st_this_vp.stats.run_queue_10ms = 0;
        _st_this_vp.stats.run_queue_100ms = 0;
        _st_this_vp.stats.run_queue_1s = 0;
        _st_this_vp.stats.run_queue_large = 0;
...
#ifdef ST_SCHEDURE_STAT
st_vp_stat_t st_get_vp_stat()
{
    return &_st_this_vp.stats;
}
#endif
/*
...
#ifdef ST_SCHEDURE_STAT
  _st_this_vp.stats.vp_cycle_elapsed += elapsed;
  ++_st_this_vp.stats.vp_cycle_num;
#endif
